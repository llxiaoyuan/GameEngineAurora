...
void SpriteRenderer::draw(Actor& actor)
{
	...
	glm::mat4 pos = glm::translate(glm::mat4(1.f), actor.position);
	defaultShader.setMatrix4fv("pos", pos);
	actor.render();
}

void SpriteRenderer::draw(BitmapFont& bitmapFont, const std::string& context, const float& x, const float& y,const float& r,const float& g,const float& b,const float& a)
{
...
void SpriteRenderer::draw(BitmapFont& bitmapFont, const char& c, const float& x, const float& y, const float& r, const float& g, const float& b, const float& a)
{
	defaultShader.setVec4f("spriteColor", r, g, b, a);
...
void SpriteRenderer::draw(Texture& texture, const float& x, const float& y, const float& originX, const float& originY, const float& rotation)
{
	...
	pos = glm::rotate(pos, rotation * Math::angToDeg, glm::vec3(0, 0, 1.f));
